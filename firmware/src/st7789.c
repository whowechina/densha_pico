...
static void st7789_boot()
{
    ...
    // You may want to set this according to your orientation logic
    uint8_t madctl = 0xc0; // Default
    // TODO: set madctl based on your orientation if needed
    send_cmd(0x36, &madctl, 1);

    uint8_t pixfmt = 0x05; // 16bit
    send_cmd(0x3A, &pixfmt, 1);

    uint8_t b2[] = {0x05, 0x05, 0x00, 0x33, 0x33};
    send_cmd(0xB2, b2, sizeof(b2));
    ...
    uint8_t c6 = 0x0F;
    send_cmd(0xC6, &c6, 1); // 60Hz dot inversion

    uint8_t d0_1[] = {0xA7, 0xA1};
    send_cmd(0xD0, d0_1, sizeof(d0_1));

    uint8_t d0_2[] = {0xA4, 0xA1};
    send_cmd(0xD0, d0_2, sizeof(d0_2));

    uint8_t d6 = 0xA1;
    send_cmd(0xD6, &d6, 1);

    uint8_t e0[] = {0xD0, 0x04, 0x08, 0x0A, 0x09, 0x05, 0x2D, 0x43, 0x49, 0x09, 0x16, 0x15, 0x26, 0x2B};
    send_cmd(0xE0, e0, sizeof(e0));

    uint8_t e1[] = {0xD0, 0x03, 0x09, 0x0A, 0x0A, 0x06, 0x2E, 0x44, 0x40, 0x3A, 0x15, 0x15, 0x26, 0x2A};
    send_cmd(0xE1, e1, sizeof(e1));

    send_cmd(0x20, NULL, 0); // Display inversion off
    send_cmd(0x13, NULL, 0); // Normal Display Mode On

    send_cmd(0x29, NULL, 0); // Display on (commented out in original)
    sleep_ms(10);
}
